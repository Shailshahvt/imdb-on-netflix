// Universal Streaming Ratings Extension - Platform Agnostic Architecture
(() => {
    'use strict';
    ...
    const BASE_CONFIG = {
        ...
        MIN_MATCH_SCORE: 0.7,
        OBSERVER_DELAY: 3000,
        IMDB_API_URL: 'https://api.imdbapi.dev/search/titles',
    ...
    const ApiService = {
        ...
        async processRequest({ title, type, cacheKey, resolve }) {
            ...
            try {
                ...
                const ratings = await this.fetchRatings(title, type, cacheKey);
                resolve(ratings);
            } catch (error) {
        ...
        async fetchRatings(title, expectedType, cacheKey, retryCount = 0) {
            try {
                // Fetch both IMDb and OMDB ratings concurrently
                const [imdbRating, omdbRating] = await Promise.allSettled([
                    this.fetchImdbRating(title, expectedType),
                    this.fetchOMDBRating(title, expectedType)
                ]);

                const ratings = {
                    imdb: imdbRating.status === 'fulfilled' ? imdbRating.value : null,
                    omdb: omdbRating.status === 'fulfilled' ? omdbRating.value : null
                };

                // Only cache if we have at least one rating
                if (ratings.imdb || ratings.omdb) {
                    this.cache[cacheKey] = {
                        data: ratings,
                        timestamp: Date.now()
                    };
                ...
                if (error.name !== 'AbortError' && retryCount < 1) {
                    ...
                    return this.fetchRatings(title, expectedType, cacheKey, retryCount + 1);
                }
            }
            return null;
        },

        async fetchImdbRating(title, expectedType, retryCount = 0) {
            try {
                const controller = new AbortController();
                const timeoutId = setTimeout(() => controller.abort(), 5000); // 5 second timeout

                const response = await fetch(
                    `${BASE_CONFIG.IMDB_API_URL}?query=${encodeURIComponent(title)}`,
                    { 
                        method: 'GET',
                ...
                if (response.status === 429) {
                    if (retryCount < 2) {
                        ...
                        return this.fetchImdbRating(title, expectedType, retryCount + 1);
                    }
                    return null;
                }
        ...
        async fetchOMDBRating(title, expectedType, retryCount = 0) {
            try {
                const controller = new AbortController();
                const timeoutId = setTimeout(() => controller.abort(), 5000); // 5 second timeout
                ...
                const OMDB_API_KEY = 'YOUR_OMDB_API_KEY'; // Get free key from omdbapi.com
                
                const response = await fetch(
                    `${BASE_CONFIG.OMDB_API_URL}?apikey=${OMDB_API_KEY}&t=${encodeURIComponent(title)}`,
                    { 
                        method: 'GET',
                        signal: controller.signal,
                        headers: {
                ...
                if (data.Response === 'True' && data.imdbRating && data.imdbRating !== 'N/A') {
                    ...
                    if (data.Ratings) {
                        ...
                        if (rtData && rtData.Value !== 'N/A') {
                            rtRating = rtData.Value.replace('%', ''); // Remove % symbol
                        }
                    }
                ...
                if (error.name !== 'AbortError' && retryCount < 1) {
                    await new Promise(resolve => setTimeout(resolve, 500));
                    return this.fetchOMDBRating(title, expectedType, retryCount + 1);
                }
            }
            return null;
        },
    ...
    const Overlay = {
        create(ratings) {
            ...
            if (ratings.imdb) {
                content += `
                    <div class="rating-item imdb-rating">
                        ...
                        <div class="rating-score">${ratings.imdb.score}</div>
                        <div class="rating-votes">${ratings.imdb.votes}</div>
                    </div>
                `;
            }
            ...
            if (ratings.omdb) {
                // Show IMDb rating from OMDB
                if (ratings.omdb.imdbScore) {
                    content += `
                        <div class="rating-item imdb-rating">
                            <div class="rating-logo">IMDb</div>
                            <div class="rating-score">${ratings.omdb.imdbScore}</div>
                            <div class="rating-votes">${ratings.omdb.imdbVotes}</div>
                        </div>
                    `;
                }
                
                // Show Rotten Tomatoes rating if available
                if (ratings.omdb.rtScore) {
                    content += `
                        <div class="rating-item rt-rating">
                            <div class="rating-logo">RT</div>
                            <div class="rating-score">${ratings.omdb.rtScore}%</div>
                            <div class="rating-votes">Critic</div>
                        </div>
                    `;
                }
                
                // Show Metascore if available
                if (ratings.omdb.metascore) {
                    content += `
                        <div class="rating-item meta-rating">
                            <div class="rating-logo">MC</div>
                            <div class="rating-score">${ratings.omdb.metascore}</div>
                            <div class="rating-votes">Critic</div>
                        </div>
                    `;
                }
    ...
    const StreamingRatings = {
        ...
        async processCard(element, titleData) {
            try {
                ...
                const ratings = await ApiService.getRatings(titleData);
                console.log('Received ratings:', ratings);
                
                if (ratings && (ratings.imdb || ratings.rottenTomatoes)) {
                    const overlay = Overlay.create(ratings);
                    Overlay.addTo(element, overlay, this.platform.config);
                } else {
                    console.log('No ratings received for:', titleData.title);
                }
            } catch (error) {
